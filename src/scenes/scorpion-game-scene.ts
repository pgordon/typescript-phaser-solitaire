...
export class ScorpionGameScene extends Phaser.Scene {
  // contains the core Solitaire game logic and has the actual game state TODO: Change to ScorpionSolitaire
  #solitaire!: ScorpionSolitaire;
  // keeps track of the card game objects in our draw pile (will have 3 game objects)
  #drawPileCards!: Phaser.GameObjects.Image[];
  // keeps track of the card game objects in our discard pile (will have 2 game objects) TODO: remove
  #discardPileCards!: Phaser.GameObjects.Image[];
  // keeps track of the card game objects in each of the foundation piles (4 game objects) TODO: remove? or could use at the very end?
  ...
  constructor() {
    super({ key: SCENE_KEYS.GAME_SCORPION });
  }
  ...
  #createDrawPile(): void {
    ...
    if (DEBUG) {
      this.add.rectangle(drawZone.x, drawZone.y, drawZone.width, drawZone.height, 0x00ffff, 0.5).setOrigin(0);
    }
    ...
    drawZone.on(Phaser.Input.Events.POINTER_DOWN, () => {
      ...
      // reaching here means we have cards in the draw pile, so we need to draw a card TODO: one time action to deal them onto first 3 tableau piles
  ...
  #createFoundationPiles(): void { //TODO: this could perhaps stay to have to do with the win condition
